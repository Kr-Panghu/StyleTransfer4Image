...
# This should be aligned with your pre-trained diffusion
model_config.update({
...
os.environ["CUDA_VISIBLE_DEVICES"] = "0,1" # May modify this
...
# Use clip.available_models() to check the available pre-trained CLIP models.
    """
    'RN50',
    'RN101',
    'RN50x4',
    ...
    'ViT-L/14@336px'
    """
# But should pay attention to the GPU memory usage.
...
parser = argparse.ArgumentParser(description='CLIP-Guided Diffusion Arguments')
parser.add_argument('--path_to_image', type=str, default='portrait.png', help='Path to the input image')
parser.add_argument('--output_dir', type=str, default='results', help='Path to the output file')
parser.add_argument('--source', type=str, help='description for source image')
parser.add_argument('--target', type=str, help='description for target image')

hargs = parser.parse_args()
p_source = hargs.source
p_target = hargs.target
batch_size = 1
...
path_to_image = hargs.path_to_image
output_dir = hargs.output_dir
init_image = Image.open(path_to_image).convert('RGB')
    ...
    for j, sample in tqdm.tqdm(enumerate(samples)):
        ...
        if j % 5 == 0 or cur_t == -1:
            ...
            for k, image in enumerate(sample['pred_xstart']):
                filename = f'{output_dir}/progress_{i * batch_size + k}.png'
                # filename = f'{output_dir}/{p_target}.png'
